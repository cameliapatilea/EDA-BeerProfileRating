...
def train_nn(X_train, y_train, X_test, y_test, model_name, num_classes):
    n_epochs = 25
    learning_rate = 1e-3
    ...
    history = classifier.fit(X_train, y_train, epochs=n_epochs, verbose=2)  # , class_weight=class_weight)
    ...
    if num_classes > 1:
        logging_metrics_list = get_classif_perf_metrics(y_test, y_pred, model_name, num_classes=num_classes)
    else:
